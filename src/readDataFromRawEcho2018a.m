function DataFileTable = readDataFromRawEcho2018a(RadarFileName)
% 这个位置是写文档用的
...
FrameHeadFlagUint = uint32(hex2dec('AAAA5555'));
FrameHeadFlagInt = typecast(FrameHeadFlagUint, 'int32');
FrameEndFlagUint = uint32(hex2dec('AA5555AA'));
...
for FrameHeadLocInd = FrameHeadLoc
    ...
    %% 解析信号带宽、采样率、信号形式、探测模式
        ...
        if SampleRateCode == 0
            SampleRateTemp = 4;
        elseif SampleRateCode == 1
            SampleRateTemp = 0.1;
        elseif SampleRateCode == 2
            SampleRateTemp = 0.2;
        elseif SampleRateCode == 3
            SampleRateTemp = 0.4;
        else
            SampleRateTemp = 0;
        end
        ...
        WaveTypeTemp = uint8(WaveTypeTemp);
        WaveTypeNameTemp = '';
        
        if WaveTypeTemp == 0
            WaveTypeNameTemp = '线性调频';
        elseif WaveTypeTemp == 1
            WaveTypeNameTemp = '互补码';
        elseif WaveTypeTemp == 2
            WaveTypeNameTemp = '巴克码';
        elseif WaveTypeTemp == 3
            WaveTypeNameTemp = '交替码';
        elseif WaveTypeTemp == 4
            WaveTypeNameTemp = '长脉冲';
        elseif WaveTypeTemp == 7
